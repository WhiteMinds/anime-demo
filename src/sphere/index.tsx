...
export function Sphere() {
  const refSphereRoot = React.useRef<HTMLDivElement>(null)

  useEffect(() => {
    if (!refSphereRoot.current) return

    const spherePathEls: SVGPathElement[] = Array.from(
      refSphereRoot.current.querySelectorAll('.sphere path')
    )
    const baseT = 300
    ...
    anime({
      ...
      strokeDasharray: [
        {
          value(el: SVGPathElement) {
            const halfLen = el.getTotalLength() / 2
            const result = [
              `0, ${halfLen}px, 0, 0`,
              `0, ${halfLen}px, ${halfLen}px, 0`,
            ]
            return result
          },
          duration: baseT,
          delay: anime.stagger(baseT * 1.25, { direction: 'reverse' }),
        },
        {
          value(el: SVGPathElement) {
            const halfLen = el.getTotalLength() / 2
            return `${halfLen}px, 0, ${halfLen}px, 0`
          },
          duration: baseT,
        },
        {
        ...
        },
        {
          value(el: SVGPathElement) {
            const halfLen = el.getTotalLength() / 2
            return [`0, 0, ${halfLen}px, ${halfLen}px`, `0, ${halfLen}px, 0, 0`]
          },
          duration: baseT,
        },
      ],
  ...
  return (
    <div className="animation-wrapper">
      <div ref={refSphereRoot} className="sphere-animation">
        <svg
          className="sphere"
          viewBox="0 0 440 440"
